...
- เพิ่มระบบ **Test Mode** และ **WiFi Config Mode** (SoftAP + Captive Portal "iCough-Setup") ควบคุมด้วยปุ่มกด GPIO 3 และแสดงไฟ LED GPIO 20
- แก้ปัญหาแรมเต็ม (OOM) โดยบันทึกไฟล์ WAV ตรงลง LittleFS และปรับพาร์ติชัน (เอา OTA ออก) ทำให้เก็บข้อมูล LittleFS ได้มากถึง 2.18 MB
- ปรับเปลี่ยนเซิร์ฟเวอร์ปลายทางมาใช้ HTTP ธรรมดา พอร์ต 8000 และรันบน Ubuntu VPS ภายใต้ PM2 (`icough-server`) ได้อย่างเสถียร
- เพิ่มเวลา `HTTP_RESPONSE_TIMEOUT_MS` เป็น 35 วินาทีในบอร์ดเพื่อรองรับเวลาวิเคราะห์โมเดล AI
- ทดสอบระบบ Batch Upload จากโฟลเดอร์ `/normal` ส่งไฟล์ไปยังเซิร์ฟเวอร์สำเร็จรวดเดียว 7 ไฟล์ (สำเร็จ 100%) พร้อมลบไฟล์ออกจาก Flash ทันทีเมื่ออัปโหลดเสร็จสิ้น
