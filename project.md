...
- เมื่อเกิด Trigger โดยระดับความดังเกินกำหนด (ตั้งไว้ที่ `-35.0f` dBFS) และวิเคราะห์ผ่านการกรองความถี่ด้วย ZCR ในช่วง `0.05` ถึง `0.32` ว่ามีแนวโน้มเป็นเสียงไอจริง จะเริ่มบันทึกคลิปเสียงความยาว 3 วินาที โดยดึงเสียงย้อนหลังจาก RAM Pre-buffer 1.2 วินาทีมารวมด้วยเพื่อป้องกันสัญญาณช่วงเริ่มไอตกหล่น
- เขียนไฟล์เสียง (.wav) ลงระบบไฟล์ LittleFS
- กลับไปฟังเสียงและเก็บลง Pre-buffer ต่อทันที
- เปิด Wi-Fi เป็นรอบเวลาทุก 10 นาที (หากตรวจเช็คว่าในโฟลเดอร์ `/normal` มีไฟล์เสียงค้างอยู่จริงเท่านั้น เพื่อลดการใช้พลังงานโดยสูญเปล่า)
- อัปโหลดไฟล์เสียงทั้งหมดในคิว
- หากส่งสำเร็จ (ได้รับรหัสสถานะ `200 OK` จากเซิร์ฟเวอร์) บอร์ดจะสั่งลบไฟล์ทิ้งทันที และเมื่อหมดคิวอัปโหลดแล้ว จะทำการตัดการเชื่อมต่อและปิด WiFi ทันที (`wifi_disconnect`)
...
## Current State
- เขียนโค้ดดึงไมค์ I2S พิมพ์ค่า RMS และ dBFS ได้แม่นยำ
- อัปเกรดระบบประหยัดพลังงานโดย **ปิด WiFi ตอนเริ่มต้น** บอร์ดจะต่อเน็ตเฉพาะตอนเข้า Test Mode หรือเมื่อครบรอบ 10 นาที (กรณีมีไฟล์เสียงในแฟลชเท่านั้น) พร้อมตัดการเชื่อมต่อและปิด WiFi ทันทีหลังอัปโหลดเสร็จสิ้น (`wifi_disconnect`)
- เพิ่มระบบตัวกรองเสียงไอในบอร์ด (**Hardware-level Cough Filter - ZCR**) คำนวณหาค่า Zero Crossing Rate ช่วงความถี่ `0.05` ถึง `0.32` ร่วมกับระดับความดังเสียง เพื่อตัดเสียงรบกวนความถี่ต่ำ/สูง (เช่น เสียงลมเป่า, เสียงพัดลม, เสียงเคาะกระแทกโต๊ะ) ก่อนบันทึกลง Flash ช่วยลดไฟล์ขยะ (False Trigger) ลงได้กว่า 50-70%
- ปรับแต่งเกณฑ์ความไวเสียง (`COUGH_THRESHOLD_DB`) จากเดิม `-30.0f` เป็น **`-35.0f` dBFS** เพื่อตอบสนองต่อเสียงไอเด็กที่เบาลงหรืออยู่ห่างออกไปได้ดียิ่งขึ้น
- แก้ไขปัญหาแรมเต็ม (OOM) และบอร์ดค้าง โดยใช้ระบบ **FIFO (First In, First Out)** เพื่อจัดการไฟล์ใน LittleFS เมื่อเต็ม บอร์ดจะลบไฟล์เก่าสุดออกเพื่อเขียนทับทันทีโดยไม่ฝืนต่อ WiFi ค้างให้เปลืองแบตเตอรี่
- เชื่อมต่อและอัปโหลดไฟล์เสียง WAV จากบอร์ดขึ้น FastAPI Server และจัดเก็บลง Supabase Storage พร้อมประวัติใน Supabase DB สำเร็จ
- ทำระบบสรุปรายงานเข้า LINE OA ทุกเช้า 06:00 น. (**Daily Summary Cron Job**) สำเร็จ โดยปรับการยิง API ตาม Timezone UTC ของเซิร์ฟเวอร์ (รันเวลา 23:00 น. UTC) เพื่อให้ตรงเวลากับเวลา 06:00 น. ของไทย
- ปรับปรุงเซิร์ฟเวอร์ FastAPI ให้แสดง **Timestamp เวลาของประเทศไทย (+07:00)** นำหน้าทุกล็อกไฟล์บน PM2 ช่วยให้ดีบั๊กได้มีประสิทธิภาพ
- หน้าเว็บแอป Next.js เชื่อมต่อข้อมูลผล AI, Spectrogram, และระบบวิเคราะห์สัญญาณไฟ override ด้วยการทำ Checklist อาการร่วมเสร็จสมบูรณ์เรียบร้อย
- มีแบบฟอร์มตรวจสอบสถานะการทดสอบระบบรวมอยู่ที่ไฟล์ `TEST_CHECKLIST.md` ใน root ของโครงการ
